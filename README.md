...
**2026-09-24 第13批开发验收范围：** 108份精确完整PDF、92家公司、106个公司/年度组合；
102份生成待人工复核候选，6份保留缺口，88家公司至少有一份候选。本批新增20家公司2025年报，
其中18份通过；原有88份的状态、三表检查和两期五项金额全部不变。人工财务确认仍为0。
可搜索股票数不等于逐份年报验证数，未测试公司、年度、文件版本不推定成功。

选择“从已测试的官方原件生成”后，每次重新下载、核SHA与页数，再执行当前解析。
本批补齐直接读取官方原件的来源记录；两源对照、JSON下载和已保存案件保留同一输入方式，
历史记录没有此字段时明确显示未记录。没有自动人审、自动保存或自动解锁正式底稿。

本批修复同仁堂双章附注、青岛啤酒合并/母公司四列、特变电工第三页利润归属；
兖矿的独立其他权益工具持有人利润按原文单列核对，归母股东原值保持不变。
明确区分母公司“不适用”斜杠与合并数值；不把空白或未知标记补零。
利润续页每次出现的期间头必须一致，独立加项必须确有并列关系，换行“其中”不能被忽略。

现金行遇未识别文字即停止，不跨下一科目借金额。已验证的显式附注表头与单个完整注释格保留，
通过美的、神华、中车、移动、青啤完整原件回归；不是恢复任意跳过文字。
其他既有专用金融机构规则、原件指纹约束、原始页片段和两期金额检查不变。

海螺资产/负债的官方重述依据已接入实时对照与历史案件展示：原年报值和后续重述值分别标明出处，
后续元级表未列调整前值时不造字段，原金额与差异状态保留。
本批新增100项本期公开对照：88项零差额、TCL中环2项保留差异、10项因整份快照未通过而不可比。
TCL差异已查明与后续半年报重述比较数对应，详细来源已留档，未加入自动解释登记。

六份未通过原件为：人保2025旧数字缺失版、洛阳钼业2025旧A股图像版、中国建筑2025图像主表、
中国联通2025受损文字层、东航2025图像财务页、古井2025空白汇率现金行。
建筑官网另链接与既有失败原件SHA相同，不重复算新覆盖；联通另链接未取得可验证原件。
此前中铁/紫金超大原件仍排除。未启用OCR，未提高手工32 MiB、官方快照45 MiB和1000页上限。

详细范围见[第13批完整年报](docs/INDUSTRY_EXPANSION_13.md)、[利润归属与四列](docs/COVERAGE13_INCOME_ATTRIBUTION_COLUMNS.md)、
[海螺双源重述](docs/CONCH_DUAL_SOURCE_RESTATEMENT.md)、[TCL重述差异](docs/COVERAGE13_TCL_RESTATEMENT.md)、
[官方原件入口](docs/TESTED_ORIGINAL_SNAPSHOT.md)。此前见[第12批](docs/INDUSTRY_EXPANSION_12.md)、
[完整原件目录](docs/ANNUAL_REPORT_COVERAGE_CATALOG.md)、[公开财务范围](docs/PUBLIC_FINANCIAL_COVERAGE.md)。
本地回归不证明已上线；线上版本须核对 `/app/static/release.json` 和本批最终部署回执。
...
证券、保险只开放本README开头明确列出的发行人和年度模板；华泰与招商2024/2025及已验证保险年报使用专项三表规则，其他版式不能据此推定支持。候选、人工复核底稿、研究案件和页面统一停算普通公司比例。北交所已验证锦波与贝特瑞2024/2025、连城2025，均仍待人工复核。

证券收入按发行人报表保留明确口径：中信使用营业收入，华泰、招商使用营业总收入；保险收入同样区分营业收入与营业总收入。人保2025旧巨潮版本数值文字层不可用，仍不生成金额候选；另行测试的港交所A股CAS原件已可生成候选，两份PDF分别保留状态。年报原披露与当前公开源重述值有差异时保留双值，具体范围与来源见本README开头链接及对应批次记录。

连续解析大年报时，每8页释放可重建的PDF库缓存，保留全部原文和版面证据；上传检查也避免创建整份文件的可写副本。既有文件、页数、文字量上限及串行解析限制不变。该改动针对实际发现的512 MB实例内存重启，验证范围和性能代价见 [PDF资源管理记录](docs/PDF_MEMORY_LIFECYCLE.md)。
