...
**2026-09-24 第14批完整开发验收范围：** 132份精确完整PDF、116家公司、130个公司／年度组合；
122份生成待人工复核候选，10份保留缺口，108家公司至少有一份候选。本批新增24家公司2025年报，
其中20份候选、4份缺口；旧108份状态、三表检查和两期五项金额、原始数值、单位与页码全部保持。
人工财务确认仍为0。可搜索股票数不等于逐份年报验证数，未测试公司、年度、文件版本不推定成功。

本批支持完整年度期间头、税前利润注释跨页、受限现金附注与连续印刷页码边界。
顺丰四列明确区分合并与公司，归属行公司栏“不适用”不补零；所得税方向由原文费用符号版式确定，
不靠金额是否算平选择。原始片段、金额、单位、物理页码及官方页链接贯穿候选、快照、JSON和HTML。
新增来源展示不会自动完成人审或解锁正式复核底稿。

TCL中环资产／负债的双源重述说明已接入：原年报原值与后续半年报重述比较值各自保留出处，
两项金额差异及待人工复核状态保持；不伪造后续金额页的调整前值，也不重写公开源原获取时间。
选择“从已测试的官方原件生成”仍须重新下载、核SHA与页数，再执行当前解析，不能用历史通过代替。

本批新增缺口为华电国际、重庆啤酒两期汇率现金行空白，深信服本期少数股东损益空白，春秋航空图像财务页。
旧六份缺口保持；其他版本成功、原生提取器局部可读或公开源有值均不能覆盖失败原件。
国投电力官方原件约74.15 MiB，超过45 MiB官方上限，未纳入；德邦本次官方查询未取得原件。
未OCR，未提高手工32 MiB、官方45 MiB或1000页上限；调查替代文件没有混入132份分母。

详细范围见[第14批完整年报](docs/INDUSTRY_EXPANSION_14.md)、[资产负债与现金布局](docs/COVERAGE14_BALANCE_CASH_LAYOUTS.md)、
[顺丰四列](docs/SF_EXPRESS_INCOME_COLUMNS.md)、[TCL双源说明](docs/TCL_DUAL_SOURCE_RESTATEMENT.md)、
[缺口调查](docs/COVERAGE14_GAP_INVESTIGATION.md)。此前见[第13批](docs/INDUSTRY_EXPANSION_13.md)、
[官方原件入口](docs/TESTED_ORIGINAL_SNAPSHOT.md)、[完整原件目录](docs/ANNUAL_REPORT_COVERAGE_CATALOG.md)。
最终132份原件检查与3,574项完整测试通过；新增120项公开对照为100项零差额、20项不可比。
本地验收与线上部署分别记录；发布状态以实际部署回执为准。
线上版本须核对 `/app/static/release.json` 和对应批次最终部署回执。
